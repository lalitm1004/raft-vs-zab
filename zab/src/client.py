        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(f"  [RETRY] Attempting to reconnect... ({attempt + 1}/{max_retries})")
                    if not self._try_connect():
    ...
    def _try_connect(self, max_attempts: int = 3) -> bool:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                print(f"  [CONNECT] Trying node {node_id} at {addr}... (attempt {attempt + 1}/{max_attempts})")
                    ...
                    if len(parts) < 2:
                        ...
                        try:
                            ...
                            if 0 <= node_id < client.config.cluster_size:
                                ...
                                print(f"Invalid node_id. Must be between 0 and {client.config.cluster_size - 1}")
                        except ValueError:
